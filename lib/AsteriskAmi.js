 ...
 * action
 *
 * This is a wrapper for send to add tracking and
 * callback queuing
 *
 */
AsteriskAmi.prototype.action = function(action,args,cb){
   if( typeof args === 'function' ) {
      cb = args;
      args = null;
   }
   var opts = { ActionID: astUtils.generateRandomActionID.call(this) };
   if(args) {
   ...
   if( action && typeof action === 'string' ) {
      ...
      // Accommodate throwing one object with
      // everything in it
      opts = _.extend(opts,action);
   }
   if( !opts.Action ) {
      return cb && cb('No Action found calling action');
   }
 ...
 * agi
 *
 * This is a wrapper for action which adds CommandID
 * tracking and dual-stage callback support.  It is
 * designed to work with AsyncAGI.
 *
 * Note that the parameter-passed callback
 * will be called when AMI _accepts_ the AGI command.
 * If you want another callback to be called when
 * the AGI command is complete, then pass it as the
 * 'complete' property of the args parameter.
 *
 */
AsteriskAmi.prototype.agi = function(action,args,cb){
   if( typeof args === 'function' ) {
      cb = args;
      args = null;
   }
   var opts = {
      Action: 'AGI',
      ActionID: astUtils.generateRandomActionID.call(this),
      CommandID: astUtils.generateRandomActionID.call(this)
   };
   ...
   if( !opts.Command ) {
      return cb && cb('No Command found when calling agi');
   }
   if( cb && typeof cb === 'function' ) {
      this.actionQueue[opts.ActionID] = cb;
   }
